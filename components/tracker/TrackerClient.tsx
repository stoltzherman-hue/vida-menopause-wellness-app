...
const METRICS: { key: Metric; label: string; color: string }[] = [
  { key: 'mood',              label: 'Mood',        color: '#9b7cc8' },
  { key: 'energy_level',      label: 'Energy',      color: '#c4b8e0' },
  { key: 'sleep_quality',     label: 'Sleep',       color: '#7a52b0' },
    ...
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8 }}>
      ...
      >
        ...
        <div style={{
          ...
          <span style={{ fontSize: 17, fontWeight: 300, color, fontFamily: 'var(--font-playfair), Georgia, serif', lineHeight: 1 }}>
            {value}
          </span>
          {isMilestone && (
            <div style={{
      ...
      <span style={{ fontSize: 11, color: 'rgba(255,255,255,0.55)', fontWeight: 300, fontFamily: 'var(--font-dm-sans), system-ui, sans-serif', textAlign: 'center' }}>{label}</span>
    </div>
  )
}
...
export function TrackerClient({ checkins, streak, monthCount }: Props) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
        ...
        <div className="milestone-card">
          <div style={{ position: 'relative', zIndex: 1, display: 'flex', alignItems: 'center', gap: 16 }}>
            <div style={{
              ...
              background: 'rgba(155,124,200,0.25)',
              border: '1px solid rgba(155,124,200,0.4)',
              display: 'flex', alignItems: 'center', justifyContent: 'center',
            }}>
              <div style={{ width: 16, height: 16, borderRadius: '50%', background: '#c4b8e0' }} />
            </div>
            <div>
              <p style={{ fontWeight: 300, fontSize: 17, color: 'white', margin: 0 }}>
                {streak}-day streak!
              </p>
      ...
      {topSymptom && (
        <div style={{
          ...
          <div style={{
            ...
            <svg width="19" height="19" viewBox="0 0 24 24" fill="none" stroke="#9b7cc8" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
              <circle cx="12" cy="12" r="10"/><path d="M12 8v4l3 3"/>
            </svg>
          </div>
          <div>
            <p style={{ fontSize: 12, fontWeight: 300, color: 'rgba(255,255,255,0.32)', letterSpacing: '0.06em', textTransform: 'uppercase', margin: '0 0 3px' }}>Your data suggests</p>
            <p style={{ fontSize: 14, fontWeight: 300, color: 'rgba(255,255,255,0.88)', margin: 0 }}>
      ...
      {/* Tabs */}
      <div style={{ display: 'flex', background: 'rgba(255,255,255,0.04)', borderRadius: 18, padding: 4, gap: 3 }}>
        {(['calendar', 'trends', 'history', 'triggers'] as const).map((t) => (
          <button key={t} onClick={() => setTab(t)} style={{
            ...
            fontSize: 12, fontWeight: tab === t ? 300 : 300,
            cursor: 'pointer',
            boxShadow: tab === t ? '0 2px 8px rgba(0,0,0,0.18)' : 'none',
      ...
      {tab === 'calendar' && (
        <div key="calendar" className="step-enter" style={card}>
          <p style={{ fontFamily: 'var(--font-playfair), Georgia, serif', fontSize: 15, fontWeight: 300, color: 'rgba(255,255,255,0.88)', marginBottom: 4 }}>
            35-day wellness calendar
          </p>
          <p style={{ fontSize: 12, color: 'rgba(255,255,255,0.32)', marginBottom: 16, fontFamily: 'var(--font-dm-sans), system-ui, sans-serif' }}>
          ...
          {checkins.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '32px 0' }}>
              ...
              <p style={{ fontSize: 14, color: 'rgba(255,255,255,0.55)', lineHeight: 1.6 }}>
                ...
                <a href="/check-in" style={{ color: '#9b7cc8', fontWeight: 300 }}>Start tracking →</a>
              </p>
            </div>
          ) : (
          ...
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 16 }}>
            {METRICS.map((m) => (
              <button key={m.key} onClick={() => setActiveMetric(m.key)} style={{
                borderRadius: 20, padding: '6px 14px', fontSize: 12, fontWeight: 300,
                border: activeMetric === m.key ? 'none' : '1px solid rgba(255,255,255,0.09)',
                background: activeMetric === m.key ? 'linear-gradient(135deg, #9b7cc8 0%, #7a52b0 100%)' : 'rgba(255,255,255,0.05)',
                color: activeMetric === m.key ? 'white' : 'rgba(255,255,255,0.55)',
      ...
      {tab === 'history' && (
        <div key="history" className="step-enter" style={card}>
          <p style={{ fontFamily: 'var(--font-playfair), Georgia, serif', fontSize: 15, fontWeight: 300, color: 'rgba(255,255,255,0.88)', marginBottom: 14 }}>Recent check-ins</p>
      ...
      {tab === 'triggers' && (
        <div key="triggers" className="step-enter" style={card}>
          <p style={{ fontFamily: 'var(--font-playfair), Georgia, serif', fontSize: 15, fontWeight: 300, color: 'rgba(255,255,255,0.88)', marginBottom: 14 }}>Trigger patterns</p>
